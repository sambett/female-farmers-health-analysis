  ...
  return (
    <div className="w-64 bg-slate text-white p-4 flex flex-col h-screen">
      <div className="text-center mb-8">
        <h1 className="text-xl font-bold tracking-tight">AgriHealth Dashboard</h1>
      </div>
      <nav className="flex-1">
        <ul>
          {tabs.map(({ id, label, icon: Icon }) => (
            <li
              key={id}
              className={`mb-2 rounded-lg ${activeTab === id ? 'bg-teal' : 'hover:bg-teal/80'} transition-colors`}
            >
              <button
